...
%**********************************************************************************/
% clear;clc
% [F,nel,w3d,infoline] = read_vrml('1.wrl');
...
%* initialise arrays & counters */
  ...
  while ( tempstr ~= -1)
     ...
     for i=1:3  %/* check for each keyword in line */
        key = deblank(keynames(i,:)); %remove the blank end for matching
...
% XYZ=w3d(1).pts;
% XYZ=round(XYZ);
% t=min(min(XYZ(:,1)),min(XYZ(:,2)));
% X=XYZ(:,1)-t+1;Y=XYZ(:,2)-t+1;Z=XYZ(:,3)-min(XYZ(:,3));
% % m=zeros(round(max(XYZ(:,1))),round(max(XYZ(:,2))));
% t=[max(X) max(Y)];%to put the center in correct position
% for i=1:size(Z)
%     m(t(1,2)-Y(i)+1,t(1,1)-X(i)+1)=Z(i);
% end
% r=max(m);
% c=max(m');
% r=[min(find(max(m))) max(find(max(m)))];
% c=[min(find(max(m'))) max(find(max(m')))];
% for i=r(1,1):r(1,2)
%     for j=c(1,1):c(1,2)
%         p(j-c(1,1)+1,i-r(1,1)+1)=m(j,i);
%     end
% end
% % p=p*255/max(p(:));
% % imshow(p')
...
% for i=1:4
%     for j=2:size(c)-1
%         if r(j)~=1 && r(j)~=size(p,1) && c(j)~=1 && c(j)~=size(p,2)
% p(r(j),c(j))=(p(r(j)-1,c(j)-1)+p(r(j)-1,c(j))+p(r(j)-1,c(j)+1)...
%     +p(r(j),c(j)-1)+p(r(j),c(j)+1)+p(r(j)+1,c(j)-1)+p(r(j)+1,c(j))+p(r(j)+1,c(j)+1))/8;
%     end
%     end
% i=i+1;
% end
